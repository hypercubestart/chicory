...
final class AotAnalyzer {
    ...
    private final List<NewValueType> tableTypes;
            ...
            switch (ins.opcode()) {
                ...
                case BR_TABLE:
                    ...
                    if (ins.labelTable().size() == 1) {
                        result.add(new AotInstruction(AotOpCode.DROP, NewValueType.I32.id()));
    ...
    private void analyzeSimple(
        ...
        switch (ins.opcode()) {
            ...
            case REF_FUNC:
                // [] -> [ref]
                stack.push(NewValueType.FuncRef);
                break;
            case REF_NULL:
                // [] -> [ref]
                stack.push(NewValueType.refTypeForId(ins.operand(0)));
                break;
            case REF_IS_NULL:
                // [ref] -> [I32]
                stack.popRef();
    ...
    private static void updateStack(TypeStack stack, FunctionType functionType) {
        for (NewValueType type : reversed(functionType.params())) {
            stack.pop(type);
        }
        for (NewValueType type : functionType.returns()) {
            stack.push(type);
        }
    }
    ...
    private Optional<AotInstruction> unwindStack(
        ...
        List<NewValueType> dropKeepTypes =
                stack.types().stream().limit(drop + keep).collect(toCollection(ArrayList::new));
        reverse(dropKeepTypes);
        dropKeepTypes.stream().mapToLong(NewValueType::id).forEach(operands::add);
    ...
    private FunctionType blockType(Instruction ins) {
        ...
        if (NewValueType.isValid(typeId)) {
            return FunctionType.returning(NewValueType.forId(typeId));
        }
        return module.typeSection().getType((int) typeId);
    }

    private static List<NewValueType> getGlobalTypes(WasmModule module) {
        var importedGlobals =
    ...
    private static List<NewValueType> getTableTypes(WasmModule module) {
        var importedTables =
                module.importSection().stream()
    ...
    private static long[] ids(List<NewValueType> types) {
        return types.stream().mapToLong(NewValueType::id).toArray();
    }
}
